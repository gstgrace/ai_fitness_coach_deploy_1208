// constants/index.js
export const AI_SOURCES = [
    { value: 'openai', title: 'OpenAI GPT' },
    { value: 'gemini', title: 'Google Gemini' }
  ];
  
  export const FITNESS_LEVELS = [
    { value: 'beginner', title: 'Beginner' },
    { value: 'intermediate', title: 'Intermediate' },
    { value: 'advanced', title: 'Advanced' }
  ];
  
  export const GENDERS = [
    { value: 'female', title: 'Female' },
    { value: 'male', title: 'Male' }
  ];
  
  export const GOALS = [
    { value: 'weight-loss', title: 'Weight Loss' },
    { value: 'muscle-gain', title: 'Muscle Gain' },
    { value: 'overall-fitness', title: 'Overall Fitness' },
    { value: 'stress-reduction', title: 'Stress Reduction' }
  ];